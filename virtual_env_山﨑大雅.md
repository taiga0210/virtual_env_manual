...
今回の手順書の構成は以下になりますので、順番に進めていきましょう。

**①導入・条件**  
環境構築のための各種アプリケーションをインストールします。

**②Vagrantのディレクトリ作成**  
仮想環境のもとになるディレクトリを作成します。

**③Vagrantの起動**  
設定ファイルを編集し実際にVagrantを起動します。

**④ゲストOSへのログイン**  
起動した仮想環境へのログインを行います。

**⑤各種ソフトのインストール**  
仮想環境内で使用するソフトのインストールを行います。

**⑥MySQLの設定とDBの作成**  
データベースの設定と作成を行います。

**⑦Nginxのインストールとブラウザ確認**  
Webブラウザをインストールし画面表示ができるようになった確認を行います。

**⑧Laravelのセットアップ**  
今回の目標になるLaravelをインストールしログイン認証で使用するテーブルも作成します

**⑨Nginxの設定変更後のLaravelの表示**  
ブラウザに表示させるための設定を行いLaravelを実際に表示させてみます。

**⑩ログイン機能の実装**  
Laravelに対してログイン機能の実装を行いユーザーを作成してみます。
...
ViirtualBoxとは、WindowsOSの中に仮想環境を構築し、別のOSをインストールすることができるようにするためのソフトです。  
今回はWindowsの中にCentOSをインストールし、CentOSの中で主な作業を行っていきます。
...
Vagrantとは、構築した仮想環境を操作するためのソフトです。  
先ほどインストールしたVirtualBoxを「CUI」で操作する、コントローラー的役割を果たします。  
VirtualBoxだけでも環境構築等はできますが、構築から始まり、管理や共有がとても難しいのです。そこで、**Vagrant**を使用することで**CUI**つまりコマンドで環境構築、設定、共有が簡単にできてしまうということになります。
...
`sudo`はrootユーザー（システム管理者）の権限を借りるコマンドです。  
今回ゲストOSへログインしているユーザーはVgrantユーザーとなるため、許可されていない操作はrootユーザーの権限を借りなければ実行ができません。  
そのため、システムに関わるような操作をする際は`sudo`コマンドで権限を借りて実行していきます。

#### yum

yumとは、CentOSを代表とするLinuxOSのRedHat系ディストリビューションと呼ばれる種類のOSで使用されているパッケージ管理ツール・コマンドです。  
`-y` は、yumコマンドのオプションの一つで、インストール実行途中に yes/no と聞かれた場合に全て自動でyesと回答してくれます。  
そのため、大量のパッケージをインストールする際に、毎回答えずに、インストールを効率化することが可能となります。
...
### 気づいたこと・学んだこと
* Windowsの場合だとコマンドの細かな違いが生じる  
* GUIとCUIの違いを改めて感じたとともに今まで自分がGUIの環境で行ってきたフォルダの削除が以外にも危険な操作だったことも感じた
* エラーが出てやり直しの繰り返しを行うことで自分が行ってきた操作が何をしているかが少しずつ分かり、操作の応用ややり直しの効率も上がっていた
* CentOSを利用しLaravelを実装している人の多さと、同じようなエラーが発生してしまっているということ
* エラーを解消している人のすごさ
* エラーが発生するとその分何度も様々な角度でトライするので成長する
* CUIの操作に少し慣れた
